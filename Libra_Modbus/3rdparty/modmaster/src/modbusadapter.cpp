...
QVariantMap ModbusAdapter::modbusReadData_simple(int slave, int functionCode, int startAddress, int noOfItems)
{
    m_result.clear();
    ...
    m_result.insert("slave",slave);
    m_result.insert("functionCode",functionCode);
    m_result.insert("startAddress",startAddress);
    m_result.insert("noOfItems",noOfItems);
    ...
    if(ret == noOfItems)
    {
        ...
        for(int i = 0; i < noOfItems; ++i)
        {
            ...
            value_list.append(QString::number(data));
        }
        m_result.insert("value",value_list);
